...
const instrumentPositions = [
  {id:'timpanos', x:230, y:100},{id:'caixa', x:400, y:85},{id:'pratos', x:520, y:85},{id:'bombo', x:670, y:100},
  {id:'trompa', x:300, y:220},{id:'trompete', x:400, y:205},{id:'trombone', x:520, y:205},{id:'tuba', x:620, y:225},
  {id:'flauta', x:330, y:340},{id:'clarinete', x:410, y:330},{id:'fagote', x:560, y:340},
  {id:'violino', x:270, y:490},{id:'viola', x:390, y:450},{id:'violoncelo', x:530, y:450},{id:'contrabaixo', x:640, y:495}
];
...
function buildInstrumentsSvg(){
  ...
  instrumentPositions.forEach(p=>{
    ...
    g.setAttribute('role','button');
    g.setAttribute('tabindex','0');
    g.setAttribute('aria-label', `${inst.nome}, ${inst.familia}`);
    g.setAttribute('title', `${inst.nome} — clique para saber mais`);
    ...
    g.addEventListener('mouseenter', ()=> document.getElementById('stageHint').textContent = `Clique para explorar ${inst.nome}`);
...
const familyInfo = {
  cordas:{title:'Família das Cordas', text:'O som nasce da vibração de uma corda tensionada. O arco (ou os dedos) mantêm essa vibração, e o corpo de madeira amplifica e colore o timbre.', items:['Violino — agudo e brilhante','Viola — médio e aveludado','Violoncelo — grave e expressivo','Contrabaixo — fundamento grave']},
...
(function percRow(){
  ...
  const perc = ['timpanos','caixa','pratos','bombo'];
  perc.forEach(id=>{
    const inst = instrumentos[id];
